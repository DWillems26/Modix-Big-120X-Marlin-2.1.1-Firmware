 ...
 */
  ...
  void Temperature::MPC_autotune() {
    ...
    LCD_MESSAGE(MSG_HEATING);
    hotend.target = 220.0f;   // So M105 looks nice //Willie's Edit for better stabilization at higher temps. Original: 200.0f
    ...
    for (;;) { // Can be interrupted with M108
      ...
      if (ELAPSED(ms, next_test_ms)) {
        ...
        if (current_temp >= 220.0f) break; //Willie's Edit. Original: 200.0f
