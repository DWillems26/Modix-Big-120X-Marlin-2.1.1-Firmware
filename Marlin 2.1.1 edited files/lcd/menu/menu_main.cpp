...
void lcd_calibration_menu(); //Willie's Edit
void lcd_calibration_inductive_menu(); //Willie's Edit
void lcd_calibration_tilt_menu(); //Willie's Edit
void _lcd_ubl_level_bed(); //Willie's Edit
void lcd_calibration_zoffset_menu(); //Willie's Edit
void lcd_calibration_tilt_lscrew(); //Willie's Edit
void lcd_calibration_tilt_rscrew(); //Willie's Edit
void zprobe_zoffset(); //Willie's Edit
void _menu_move_z(); //willie's edit
void _submenu_printtestline(); //Willie's Edit
...
void menu_main() {
  ...
  if (busy) {
    ...
    GCODES_ITEM(MSG_AUTO_HOME, FPSTR(G28_STR)); //Willie's Edit
  ...
  //
    ...
    FILAMENT_CHANGE_ITEM();
     ...
     //Willie"s Edit - Add calibration menu to main menu
  #if(X_BED_SIZE==1200)
      SUBMENU(MSG_CALIBRATION_120X, lcd_calibration_menu); // 120X Calibration Menu
    #elif
      SUBMENU(MSG_CALIBRATION, lcd_calibration_menu); // 60 Calibration Menu
  #endif
...
//Willie's Edit to END
   /*********************************************
   **********************************************
   ***** Custom Modix "Calibration" submenu******
   **********************************************
   *********************************************/
  ...
  void lcd_calibration_menu() {
    START_MENU();
    BACK_ITEM(MSG_MAIN);
    SUBMENU(MSG_PROBE_CALIBRATION, lcd_calibration_inductive_menu);
    SUBMENU(MSG_TILT_CALIBRATION, lcd_calibration_tilt_menu);
    SUBMENU(MSG_UBL_LEVEL_BED, _lcd_ubl_level_bed);
    SUBMENU(MSG_ZOFFSET_CALIBRATION, lcd_calibration_zoffset_menu);
    END_MENU();
  }
  ...
  /**
   *
   * "Inductive Sensor Calibration" submenu
   *
   */
  void lcd_calibration_inductive_menu()
  {
    START_MENU();
    BACK_ITEM(MSG_BACK);
    GCODES_ITEM(MSG_START_PROCESS, F("G28 X Y\n G1 X300 Y50 F5000\n M211 S0\n G28 Z"));//M211 software endstops
    SUBMENU(MSG_ADJUST_HEIGHTZ, menu_move); 
    GCODES_ITEM(MSG_SAVE_SETTINGSZ, F("M211 S1\n G28"));
    END_MENU();
  }
...
/**
   *
   * "Tilt Calibration" submenu
   *
   */
  
  void lcd_calibration_tilt_menu() {
    START_MENU();
    BACK_ITEM(MSG_BACK);
    if(X_BED_SIZE==610)
      GCODES_ITEM(MSG_START_PROCESS, F("G28 X Y\n G1 X300 Y300 F5000"));
      ...
      GCODES_ITEM(MSG_START_PROCESS, F("G28 X Y\n G1 X600 Y300 F5000"));
    SUBMENU(MSG_ADJUST_HEIGHTZ, menu_move);  
    SUBMENU(MSG_CALIBRATION_LEFT, lcd_calibration_tilt_lscrew);
    SUBMENU(MSG_CALIBRATION_RIGHT, lcd_calibration_tilt_rscrew);
    END_MENU();
  }


  void lcd_calibration_tilt_lscrew(){
    START_MENU();
    BACK_ITEM(MSG_BACK);
    if(X_BED_SIZE==610)
      GCODES_ITEM(MSG_GOTOLEFTSCREW, F("G1 X0 Y300 F5000\n M84"));
    else
    {
      GCODES_ITEM(MSG_GOTOLEFTREARSCREW, F("G1 X0 Y600 F5000\n M84"));
      GCODES_ITEM(MSG_GOTOLEFTFRONTSCREW, F("G1 X0 Y30 F5000\n M84"));
    }
    END_MENU();
  }

  void lcd_calibration_tilt_rscrew(){
    START_MENU();
    BACK_ITEM(MSG_BACK);
    if(X_BED_SIZE==610)
    {
      GCODES_ITEM(MSG_GOTORIGHTREARSCREW, F("G1 X610 Y600 F5000\n M84"));
      GCODES_ITEM(MSG_GOTORIGHTFRONTSCREW, F("G1 X610 Y30 F5000\n M84"));
    }
    else
    {
      GCODES_ITEM(MSG_GOTORIGHTREARSCREW, F("G1 X1200 Y600 F5000\n M84"));
      GCODES_ITEM(MSG_GOTORIGHTFRONTSCREW, F("G1 X1200 Y30 F5000\n M84"));
    }
    END_MENU();
  }
...
/**
   ...
   *
   */
  void lcd_calibration_zoffset_menu() {
    START_MENU();
    BACK_ITEM(MSG_BACK);
    editable.celsius = thermalManager.temp_hotend[0].target;
    EDIT_ITEM_FAST(int3, MSG_1SET_TEMP, &editable.celsius, 0, thermalManager.hotend_max_target(0), []{ thermalManager.setTargetHotend(editable.celsius, 0); });
    EDIT_ITEM(LCD_Z_OFFSET_TYPE, MSG_2SETZOFFSET, &probe.offset.z, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX);
    GCODES_ITEM(MSG_3HOMEXY, F("G28 X Y"));
    GCODES_ITEM(MSG_4VERIFYHEIGHT, F("G28 Z\n G1 X0 Y20 F5000\n G1 Z0.2"));
      {
        if (thermalManager.tooColdToExtrude(active_extruder))
          ACTION_ITEM(MSG_5HEATINGPLEASEWAIT, nullptr);  // extruder too cold
        else
          GCODES_ITEM(MSG_5PRINTTESTLINE, F("G92 E0\n G1 X200 E35 F500\n G92 E0\n G1 F1000 E-1\n G28 X Y"));
      }
    ACTION_ITEM(MSG_6SAVEZOFFSET, ui.store_settings);
