...
void _lcd_ubl_validate_pla();
void _lcd_ubl_validate_petg();
...
void _menu_ubl_height_adjust() {
  ...
  //ACTION_ITEM(MSG_INFO_SCREEN, ui.return_to_status); //Willie's Edit - Save space and not needed
  END_MENU();
}
...
void _lcd_ubl_edit_mesh() {
  ...
  void _lcd_ubl_validate_mesh() {
    START_MENU();
    BACK_ITEM(MSG_UBL_TOOLS);
    SUBMENU(MSG_vALIDATE_MESH_PLA, _lcd_ubl_validate_pla); //Willie's Edit - added
    SUBMENU(MSG_VALIDATE_MESH_PETG, _lcd_ubl_validate_petg); //Willie's Edit - added
    //Willie's Edit - Removed and added menu's to specify nozzle size. Also removed custom validation menu item
    /*#if HAS_PREHEAT
      #if HAS_HEATED_BED
...
void _lcd_ubl_validate_pla() { //Willie's Edit - Added Menu to specify nozzle size
  START_MENU();
  BACK_ITEM(MSG_BACK);
  GCODES_ITEM(MSG_PLA4, F("G28\nG26 C S0.4 H205 L0.2"));
  GCODES_ITEM(MSG_PLA6, F("G28\nG26 C S0.6 H210 L0.3"));
  GCODES_ITEM(MSG_PLA8, F("G28\nG26 C S0.8 H210 L0.4"));
  GCODES_ITEM(MSG_PLA1, F("G28\nG26 C S1.0 H215 L0.5"));
  ACTION_ITEM(MSG_INFO_SCREEN, ui.return_to_status);
  END_MENU();
}

void _lcd_ubl_validate_petg() { //Willie's Edit - Added Menu
  START_MENU();
  BACK_ITEM(MSG_BACK);
  GCODES_ITEM(MSG_PETG4, F("G28\nG26 C S0.4 H240 L0.2"));
  GCODES_ITEM(MSG_PETG6, F("G28\nG26 C S0.6 H240 L0.3"));
  GCODES_ITEM(MSG_PETG8, F("G28\nG26 C S0.8 H245 L0.4"));
  GCODES_ITEM(MSG_PETG1, F("G28\nG26 C S1.0 H250 L0.5"));
  ACTION_ITEM(MSG_INFO_SCREEN, ui.return_to_status);
  END_MENU();
}
...
void _lcd_ubl_mesh_leveling() {
  START_MENU();
  BACK_ITEM(MSG_UBL_TOOLS);
  GCODES_ITEM(MSG_UBL_3POINT_MESH_LEVELING, F("G29J2")); //Willie's Edits - changed G29J0 to G29J2 for 4 point leveling
...
void _lcd_ubl_build_mesh() {
  ...
  //GCODES_ITEM(MSG_UBL_BUILD_COLD_MESH, F("G29NP1")); //Willie's Edit - Redundant Menu
  SUBMENU(MSG_UBL_FILLIN_MESH, _menu_ubl_fillin);
  //GCODES_ITEM(MSG_UBL_CONTINUE_MESH, F("G29P1C")); //Willie's Edit - Not needed. Just restart the process
  //ACTION_ITEM(MSG_UBL_INVALIDATE_ALL, _lcd_ubl_invalidate); //Willie's Edit
  //GCODES_ITEM(MSG_UBL_INVALIDATE_CLOSEST, F("G29I")); //Willie's Edit
  //ACTION_ITEM(MSG_INFO_SCREEN, ui.return_to_status); //Willie's Edit
  END_MENU();
}
...
void _menu_ubl_tools() {
  ...
  //GCODES_ITEM(MSG_UBL_MANUAL_MESH, F("G29I999\nG29P2BT0")); //Willie's Edit - Don't need manual build with BLTouch/Probe
  //#if ENABLED(G26_MESH_VALIDATION)
    //SUBMENU(MSG_UBL_VALIDATE_MESH_MENU, _lcd_ubl_validate_mesh); //Willie's Edit - Redundant Menu Item
  //#endif
